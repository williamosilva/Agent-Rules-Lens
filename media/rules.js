// Sidebar renderer. Every string comes from the extension host through
// postMessage and is written with textContent, so nothing is ever parsed as
// HTML and no dictionary lives here. Section open/closed state is kept in the
// webview state so it survives a re-render, including a language change.
(function () {
  'use strict';
  ...
  /**
   * A mark, as two local images. CSS shows the one that fits the current theme,
   * so switching theme needs no round trip. Falls back to the neutral agent
   * mark when an id has no file.
   */
  ...
  function isCollapsed(id, defaultExpanded) {
  ...
  function details(id, defaultExpanded) {
    ...
    node.open = !isCollapsed(id, defaultExpanded);
  ...
  function ruleButton(rule) {
    ...
    button.addEventListener('click', function () {
      vscode.postMessage({ type: 'openRule', fsPath: rule.fsPath });
    });
    return button;
  }
  ...
  function artifactButton(artifact) {
    ...
    button.addEventListener('click', function () {
      vscode.postMessage({ type: 'openRule', fsPath: artifact.fsPath });
    });
    return button;
  }
  ...
  function render(model) {
    ...
    if (model.header) {
      root.appendChild(header(model.header, model.language));
  ...
  window.addEventListener('message', function (event) {
    ...
    if (message.icons && typeof message.icons === 'object') {
      iconUris = message.icons;
    }
